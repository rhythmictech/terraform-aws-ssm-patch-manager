...
| <a name="input_install_schedule"></a> [install\_schedule](#input\_install\_schedule) | 6-field Cron expression describing the install maintenance schedule | `string` | `null` | no |
...
| <a name="input_patch_operation"></a> [patch\_operation](#input\_patch\_operation) | Specify if the patch manager should scan only or if it should scan and install (SCAN/SCAN\_INSTALL) | `string` | `"SCAN_INSTALL"` | no |
